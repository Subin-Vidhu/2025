...
# KMP algorithm is another algorithm to solve the pattern matching problem
...
def kmp_pattern_matching(string, pattern):
    """
    Find all occurrences of the pattern in the string using the KMP algorithm.
    ...
    Returns:
        list: The indices where the pattern is found in the string.
    """
    ...
    for i in range(len(string)):
        ...
        if string[i] == pattern[j]:
            j += 1
        if j == len(pattern):
            matched_indices.append(i - j + 1)
            j = prefix[j - 1]
    return matched_indices

string = "AABAACAADAABAABA"
pattern = "AABA"
print(f'Pattern found at {kmp_pattern_matching(string, pattern)}')